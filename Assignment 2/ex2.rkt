...
(define generate-type-vars
  (lambda(symbol)
    (cond 
          ((lambda? symbol)
             (cons (list symbol (fresh-name 'T))
                   (generate-type-vars (lambda-body symbol))))
          
          ((atomic? symbol)
             (cons (list symbol (fresh-name 'T)) (list)))
          (else
           (cons (list (car symbol) (fresh-name 'T))
                 (generate-type-vars (cdar symbol)))))))
                 ;(generate-type-vars (cdar symbol)))))))

(generate-type-vars '(lambda(x) (+ x 1)))
