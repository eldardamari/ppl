...
; A frame is a mapping function from variables (symbols) to values. 
; It is now implemented as a value that uses lists to store variable bindings
; Type: [LIST(Symbol)*LIST -> [LIST(PAIR(Symbol,T) union {empty})]]
(define make-frame 
...
; Type: [ PAIR(Symbol,T)*LIST(PAIR(Symbol,T)) -> [PAIR(Symbol,T) union {empty}] ]
(define add-binding-to-frame 
  (lambda (binding frame)
    (make-binding binding frame)
    ))

;Signature:get-all-vars(unboxed-frame)
;Purpose: returns a list of all the variables in the given frame.
;Type: [ [Symbol -> PAIR(Symbol,T) union {empty}] -> [LIST] ]
;Tests:    (derive-eval '(define f (let ((a 1) (b 2)) (lambda () (+ a b)))))
;          (get-all-vars
;           (first-frame           
...
; Signature:get-all-vals(unboxed-frame)
; Purpose: returns a list of all the values in the first frame in the given environment.
; Type: [ [Symbol -> PAIR(Symbol,T) union {empty}] -> [LIST] ]
;Tests:    (derive-eval '(define f (let ((a 1) (b 2)) (lambda () (+ a b)))))
;          (get-all-vals
;           (first-frame           
...
;Signature:  select-var(frame var)
;Purpose: returns the binding of all requested var from the list.
;Type:[LIST(PAIR(Symbol,T)*Symbol -> PAIR(Symbol,T)]
;Tests: (derive-eval '(define f (let ((a 1) (b 2)) (lambda () (+ a b)))))
;       (select-var (first-frame
;         (procedure-environment 
;          (lookup-variable-value 
;           'f 
;           the-global-environment))) 'a) ==> '(a . 1)
(define select-var
  (lambda (frame var)
