...
(define eval-lambda
  (lambda (exp env)
    (if (variadic-lambda? exp)
        (make-variadic-procedure (lambda-parameters exp)
                                 (lambda-body exp)
                                 env)
